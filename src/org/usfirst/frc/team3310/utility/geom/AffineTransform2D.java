...
public class AffineTransform2D {
	 ...
	 */
	public static AffineTransform2D createLineReflection(AbstractLine2D line) {
	...
	public static AffineTransform2D createRotation(double cx, double cy,
		...
		if (Math.abs(k * Math.PI / 2 - angle) < EqualUtils.ACCURACY) {
	...
	public static boolean isIdentity(AffineTransform2D trans) {
		if (Math.abs(trans.m00 - 1) > EqualUtils.ACCURACY)
			return false;
		if (Math.abs(trans.m01) > EqualUtils.ACCURACY)
			return false;
		if (Math.abs(trans.m02) > EqualUtils.ACCURACY)
			return false;
		if (Math.abs(trans.m10) > EqualUtils.ACCURACY)
			return false;
		if (Math.abs(trans.m11 - 1) > EqualUtils.ACCURACY)
			return false;
		if (Math.abs(trans.m12) > EqualUtils.ACCURACY)
			return false;
		return true;
	}
	...
	public static boolean isIsometry(AffineTransform2D trans) {
		...
		if (Math.abs(a * a + b * b - 1) > EqualUtils.ACCURACY)
			return false;
		if (Math.abs(c * c + d * d - 1) > EqualUtils.ACCURACY)
			return false;
		
		// determinant must be -1 or +1
		if (Math.abs(a * b + c * d) > EqualUtils.ACCURACY)
			return false;
	...
	public static boolean isSimilarity(AffineTransform2D trans) {
		...
		// test each condition
		if (Math.abs(a * a + b * b - k2) > EqualUtils.ACCURACY)
			return false;
		if (Math.abs(c * c + d * d - k2) > EqualUtils.ACCURACY)
			return false;
		if (Math.abs(a * a + c * c - k2) > EqualUtils.ACCURACY)
			return false;
		if (Math.abs(b * b + d * d - k2) > EqualUtils.ACCURACY)
			return false;
	...
	public AffineTransform2D invert() {
		...
		if (Math.abs(det) < EqualUtils.ACCURACY)
